// Revision: $Id: convertPolyhedronDemo.cpp,v 1.0 2025/04/07 17:05:20 Leon Exp $
/*
...
void MeshDemo::convertToPolyhedralMesh(const std::string& path)
{
    ...
    PREPRO_BASE_NAMESPACE::PFStatus status = pfMesh->convertToPolyhedralMesh();
    if (status != PREPRO_BASE_NAMESPACE::PFStatus::EOkay)
    {
        std::cout << "Convert to Polyhedral mesh failed." << std::endl;
        return;
    }
